             ...
             <form class="mt-8" action="<?= BASE_URL ?>?act=addToBill" method="POST">
                 <div>
                     ...
                     <div class="grid md:grid-cols-2 gap-4">
                         <div>
                             <input type="text" placeholder="Tên người nhận" name="user_name" value="<?=$billData['user_name']?>" disabled
                                 class="px-4 py-3 bg-gray-100 focus:bg-transparent text-gray-800 w-full text-sm rounded-md focus:outline-blue-600" />
                         </div>
                         <div>
                             <input type="email" placeholder="Email" name="user_email" value="<?=$billData['user_email']?>" disabled
                                 class="px-4 py-3 bg-gray-100 focus:bg-transparent text-gray-800 w-full text-sm rounded-md focus:outline-blue-600" />
                         </div>
                         <div>
                             <input type="number" placeholder="Số điện thoại nhận hàng" name="user_phone" value="<?=$billData['user_phone']?>" disabled
                                 class="px-4 py-3 bg-gray-100 focus:bg-transparent text-gray-800 w-full text-sm rounded-md focus:outline-blue-600" />
                         </div>
                         <div>
                             <input type="text" placeholder="Địa chỉ giao hàng" name="user_address" value="<?=$billData['user_address']?>" disabled
                                 class="px-4 py-3 bg-gray-100 focus:bg-transparent text-gray-800 w-full text-sm rounded-md focus:outline-blue-600" />
                         </div>
                         <div>
                 ...
                 <div class="flex gap-4 max-md:flex-col mt-8">
                     ...
                     <a href="?act=bills-delete&id=<?= $item['bill_id'] ?>" onclick="return confirm('Bạn có chắc chắn muốn hủy thanh toán này không?')">
                        <button type="button" 
